...
import { ConnectionPackageData, NewCooperativeData, Member } from './document.types';
...
export function numberToText(num: number): string {
  ...
  if (num >= 1000) {
    ...
    if (thousandsPart < 10) {
      ...
      if (thousandsHundreds > 0) result += hundreds[thousandsHundreds] + ' ';
      if (thousandsTens >= 2) result += tens[thousandsTens] + ' ';
      if (thousandsOnes > 0 || thousandsTens < 2) {
        if (thousandsTens < 2) result += ones[thousandsPart % 100];
        else result += ones[thousandsOnes];
      }
      result += ' тысяч';
    }
    result += ' ';
...
export function processConnectionPackageData(inputData: Partial<ConnectionPackageData>): ConnectionPackageData {
...
export function processNewCooperativeData(inputData: Partial<NewCooperativeData>): NewCooperativeData {
...
export function validateINN(inn: string): boolean {
  ...
  if (inn.length === 10) {
    const checksum = (
      digits[0] * 2 + digits[1] * 4 + digits[2] * 10 + digits[3] * 3 +
  ...
  if (inn.length === 12) {
    const checksum1 = (
      digits[0] * 7 + digits[1] * 2 + digits[2] * 4 + digits[3] * 10 +
      digits[4] * 3 + digits[5] * 5 + digits[6] * 9 + digits[7] * 4 +
      digits[8] * 6 + digits[9] * 8
    ) % 11;
    
    const checksum2 = (
      digits[0] * 3 + digits[1] * 7 + digits[2] * 2 + digits[3] * 4 +
      digits[4] * 10 + digits[5] * 3 + digits[6] * 5 + digits[7] * 9 +
      digits[8] * 4 + digits[9] * 6 + digits[10] * 8
    ) % 11;
